;; This is a copy of `racket/treelist` that is used for versions
;; before 8.12.0.7
...
(meta-if-version-at-least
 "8.11.1.10"
 (require (only-in '#%kernel vector-copy vector*-copy vector*-append vector*-set/copy))
 (begin
   (require (only-in racket/vector vector-copy vector-append))
   (define vector*-copy vector-copy)
   (define vector*-append vector-append)
   (define vector*-set/copy
     (lambda (vec i val)
...
(module+ unsafe
  ...
  (provide unsafe-root-add
           (rename-out [empty-node unsafe-empty-root] [treelist unsafe-treelist])
    ...
    [(Node children sizes)
     (let ([cs children]
       ...
       (unless (variable-reference-from-unsafe? (#%variable-reference))
         #;(unless (vector? cs)
             (error 'Node "bad children vector: ~v" cs))
         (unless (or (not szs) (vector? szs))
           (error 'Node "bad sizes vector: ~v" szs)))
       (if szs
           (cons cs szs)
           cs))]))
...
(define (vector*-drop vec n)
  (vector*-copy vec n (vector*-length vec)))
(define (vector*-drop-right vec n)
  (vector*-copy vec 0 (- (vector*-length vec) n)))
(define (vector*-add-left val a)
  (vector*-append (vector val) a))
(define (vector*-add-right a val)
...
(define (node-size n)
  (assert-node n)
  (vector*-length (node-children n)))
(define (node-first n)
  (assert-node n)
  (vector*-ref (node-children n) 0))
(define (node-last n)
  (assert-node n)
  (let ([cs (node-children n)]) (vector*-ref cs (fx- (vector*-length cs) 1))))
(define (node-ref n i)
  (assert-node n)
  (vector*-ref (node-children n) i))
(define (node-child-set n i v)
  (assert-node n)
  (vector*-set/copy (node-children n) i v))
...
(struct treelist (root size height)
  ...
  (lambda (tl port mode) (treelist-print "treelist" tl port mode))
  #:property prop:equal+hash
  (list (lambda (tl other-tl recur) (treelist-equal? tl other-tl recur))
        (lambda (v recur) (treelist-hash-code v recur))
        (lambda (v recur) (treelist-size v)))
  #:property prop:sequence
  (lambda (tl) (in-treelist/proc tl))
  #:property prop:stream
  (vector (lambda (tl) (treelist-empty? tl))
     ...
     [[(d) (_ tl-expr)]
      #'[(d)
         (:do-in ([(tl) tl-expr])
                 ...
                 ([pos 0] [node empty-node] [node-pos 0])
                 (pos . fx< . size)
      ...
      [(_ binds body0 body ...)
       (with-syntax ([((pre-body ...) (post-body ...)) (split-for-body stx #'(body0 body ...))]
         ...
         (for/fold/derived #,stx
                           ([root empty-node] [size 0]
                                              [height 0]
                                              #:result (wrap-result (if (fx= size 0)
                                                                        empty-treelist
                                                                        (treelist root size height))))
                           binds
                           pre-body ...
                           (unsafe-root-add root
                                            size
                                            height
                                            (let ()
                                              post-body ...))))])))

(define-syntax for/treelist (make-for/treelist #'for/fold/derived #'values))
(define-syntax for*/treelist (make-for/treelist #'for*/fold/derived #'values))
...
(define in-treelist/proc
  ...
  (let ([in-treelist (lambda (tl)
                       ...
                       (make-do-sequence (lambda ()
                                           (values (lambda (i) (treelist-ref tl i))
                                                   (lambda (i) (fx+ i 1))
                                                   0
  ...
  (cond
    ...
    [else
     (let walk ([node (treelist-root tl)]
       ...
       (cond
         [(fx= height 0) (values node (bitwise-and index MASK))]
...
(define (treelist-equal? tl other-tl recur)
  (cond
    [(or (impersonator? tl) (impersonator? other-tl)) (treelist-equal?/slow tl other-tl recur)]
    [else
     (define len (treelist-size tl))
     (cond
       [(not (fx= len (treelist-size other-tl))) #f]
       [else
        ...
        (let loop ([i 0]
          ...
          (cond
            ...
            [(a-pos . >= . (node-size a-node))
             (cond
               [(and (b-pos . >= . (node-size b-node)) (shared-subtree-size tl other-tl i))
                =>
                (lambda (len) (loop (fx+ i len) empty-node 0 empty-node 0))]
               [else
...
(define (shared-subtree-size tl other-tl index)
  (let walk ([node (treelist-root tl)]
    ...
    (cond
      [(fx= height 0) (and (eq? node other-node) (node*-size node))]
      [(eq? node other-node) (size-subtree node height)]
      [(not (fx= other-height height))
       (cond
         [(fx< other-height height)
          (define-values (bi si)
            (if (node-leftwise-dense? node)
                (values (radix index height) index)
                (step node index height)))
          (walk (node-ref node bi) other-node si other-index (fx- height 1) other-height)]
         [else (walk other-node node other-index index other-height height)])]
      [else
       (define-values (bi si)
         (if (node-leftwise-dense? node)
             (values (radix index height) index)
             (step node index height)))
       (define-values (other-bi other-si)
         (if (node-leftwise-dense? other-node)
...
(define (treelist-hash-code tl recur)
  ...
  (cond
    ...
    [else
     ;; first 16
     (hash-code-combine (for/fold ([hc 0]) ([i (in-range 0 16)])
...
(define (treelist-set tl index el)
  (cond
    [(impersonator? tl) (treelist-set/slow tl index el)]
    [else
     ...
     (define new-node
       (let set ([node (treelist-root tl)]
                 ...
                 [height height])
         (cond
           [(fx= height 0) (node-child-set node (radix index height) el)]
           [(node-leftwise-dense? node)
           ...
           [else
            ...
            (Node (node-child-set node
                                  branch-index
                                  (set (node-ref node branch-index) subindex el (fx- height 1)))
                  (node-sizes node))])))
...
(define (treelist-add tl el)
  (cond
    [(impersonator? tl) (treelist-add/slow tl el)]
    [else
     (check-treelist 'treelist-add tl)
     (define size (treelist-size tl))
     (cond
       [(fx= size 0) (treelist (leaf el) 1 0)]
       [else
        ...
        (if new-root
            ...
            (treelist (Node (vector root (new-branch el height))) (fx+ size 1) (fx+ height 1)))])]))
...
(define (unsafe-root-add root size height el)
  ...
  (if new-root
      ...
      (values (Node (vector root (new-branch el height))) (fx+ size 1) (fx+ height 1))))

(define (treelist->vector tl)
  (check-treelist 'treelist->vector tl)
  (for/vector #:length (treelist-size tl)
              ([el (in-treelist tl)])
    el))
...
(define (vector->treelist vec-in)
  ...
  (define vec
    (if (impersonator? vec-in)
        (and (vector? vec-in) (vector-copy vec-in))
        (and (vector? vec-in) ; amounts to `vector*?`
             vec-in)))
  (unless vec
    (raise-argument-error* 'vector->treelist 'racket/primitive "vector?" vec-in))
         ...
         [else
          ...
          (values
           (let loop ([start 0]
                      [height height])
             (cond
               [(fx= height 0) (Node (vector*-copy vec start (fxmin size (fx+ start MAX_WIDTH))))]
               [else
                ...
                (Node (for/vector #:length len
                                  ([start (in-range start end step)])
...
minimum required storage. |#
  ...
  (unless (fixnum? size)
    (raise (make-exn:fail:out-of-memory (format "out of memory making treelist\n  size: ~s" size)
    ...
    [else
     ...
     (define root
       (let loop ([first-one-at-this-level #f]
                  ...
                  [height height])
         (cond
           [(and first-one-at-this-level (fx= (fx- end start) first-one-size))
           ...
           [else
            ...
            (Node (for/vector #:length len
                              ([inner-start (in-range start end step)])
...
(define (list->treelist lst)
  (unless (list? lst)
    (raise-argument-error* 'list->treelist 'racket/primitive "list?" lst))
  (if (null? lst)
      empty-treelist
       ...
       [else
        ...
        (define new-root
          (let take ([node (treelist-root tl)]
                     ...
                     [height height])
            (cond
              [(fx= height 0) (Node (vector*-take (node-children node) (fx+ (radix index 0) 1)))]
              [(node-leftwise-dense? node)
               (define branch-index (radix index height))
              ...
              [else
               ...
               (cond
                 ...
                 [(fx= 1 (vector*-length new-children)) (Node new-children)]
                 [else
...
(define (treelist-drop tl pos)
  (cond
    [(impersonator? tl) (treelist-drop/slow tl pos)]
    [else
       ...
       [else
        ...
        (define new-root
          (let drop ([node (treelist-root tl)]
                     ...
                     [height height])
            (cond
              [(fx= height 0) (Node (vector*-drop (node-children node) (radix index 0)))]
              [(node-leftwise-dense? node)
               (define branch-index (radix index height))
               (define children (node*-children node))
               ...
               (cond
                 ;; one child => leftwise dense
                 [(fx= new-len 1) (Node new-children)]
                 [else
                  ...
                  (cond
                    ...
                    [(fx= size0 step) (Node new-children)]
                    [else
                     (define new-sizes (make-vector new-len size0))
                     (for ([i (in-range 0 (fx- new-len 1))])
                       (vector*-set! new-sizes i (fx+ size0 (fx* i step))))
                     (define sizeN
                       (size-subtree (vector*-ref new-children (fx- new-len 1)) (fx- height 1)))
                     (vector*-set! new-sizes
                                   (fx- new-len 1)
                                   (fx+ size0 (fx* (fx- new-len 2) step) sizeN))
                     (Node new-children new-sizes)])])]
              [else
               (define-values (branch-index subindex) (step node index height))
               (define children (node-children node))
...
(define (treelist-sublist tl pos end-pos)
  ...
  (cond
    [(and (fx= len 0) (not (impersonator? tl))) empty-treelist]
    [(and (fx= len 1) (not (impersonator? tl))) (build-treelist (treelist-ref tl pos))]
...
(define (treelist-insert tl at el)
  (cond
    [(impersonator? tl) (treelist-insert/slow tl at el)]
    [else
     ...
     (cond
       ...
       [else (treelist-append (treelist-add (treelist-take tl at) el) (treelist-drop tl at))])]))

(define (treelist-delete tl at)
  (cond
    [(impersonator? tl) (treelist-delete/slow tl at)]
    [else
     ...
     (cond
       ...
       [(fx= at (fx- size 1))
        (if (fx= at 0)
            empty-treelist
            (treelist-take tl at))]
       [else (treelist-append (treelist-take tl at) (treelist-drop tl (fx+ at 1)))])]))

(define (treelist-cons tl el)
  (cond
    [(impersonator? tl) (treelist-cons/slow tl el)]
    [else
     (check-treelist 'treelist-cons tl)
     (define size (treelist-size tl))
     (cond
       [(fx= 0 size) (treelist (leaf el) 1 0)]
       [else
        ...
        (define new-root
          (let insert-left ([a (treelist-root tl)]
            ...
            (cond
              [(fx= height 0)
               (and ((node-size a) . < . MAX_WIDTH) (Node (vector*-add-left el (node-children a))))]
              [else
               ...
               (and left
                    (Node (vector*-set/copy (node-children a) 0 left)
                          (let ([sizes (node-sizes a)])
                            (for/vector #:length (vector*-length sizes)
                                        ([n (in-vector sizes)])
                              (fx+ n 1)))))])))
        (cond
          [new-root (treelist new-root (fx+ size 1) height)]
          [else (treelist-append (treelist (leaf el) 1 0) tl)])])]))

(define treelist-append
  (case-lambda
    [(tl rhs)
     (cond
       [(or (impersonator? tl) (impersonator? rhs)) (treelist-append/slow tl rhs)]
       [else
          ...
          [else
           (define-values (new-children new-height)
             (concat-subtree (treelist-root tl)
                             ...
                             (treelist-root rhs)
                             (treelist-height rhs)))
           (treelist new-children (fx+ (treelist-size tl) (treelist-size rhs)) new-height)])])]
    [() empty-treelist]
    [(tl)
     (check-treelist 'treelist-append tl)
     tl]
...
(define (squash node new-size new-height)
  (cond
    [(and (fx= (node-size node) 1) (fx> new-height 0))
     (squash (node-first node) new-size (fx- new-height 1))]
    [else (treelist node new-size new-height)]))

;; result height is either max of two heights or one more
;; than the max of the heights
(define (concat-subtree left height-l right height-r)
  ;; only trees of the same height can be concatenated
  (cond
    [(fx> height-l height-r)
     (define-values (mid height-m) (concat-subtree (node-last left) (fx- height-l 1) right height-r))
     (rebalance left mid #f height-l height-m)]
    [(fx< height-l height-r)
     (define-values (mid height-m) (concat-subtree left height-l (node-first right) (fx- height-r 1)))
     (rebalance #f mid right height-r height-m)]
    [(fx= height-l 0)
     (cond
       [(fx<= (fx+ (node-size left) (node-size right)) MAX_WIDTH)
        (values (Node (vector*-append (node-children left) (node-children right))) 0)]
       [else
    ...
    [else
     ...
     (define-values (mid height-m)
       (concat-subtree (node-last left) (fx- height-l 1) (node-first right) (fx- height-r 1)))
     (rebalance left mid right height-l height-m)]))
...
(define (rebalance left center right height height_c)
  (define all-slots
    (merge-nodes left
  ...
  (cond
    ...
    [else
     ...
     (define-values (sum leftwise-dense?)
       (for/fold ([sum 0]
                  [leftwise-dense? #t])
                 ([i (in-range 0 (vector*-length children))])
...
(define (distribute plan target count [node-idx 0])
  (cond
    [(fx>= target count) (vector*-take plan count)]
    [else
...
(define (exec-concat-plan slots plan height)
  (cond
    ...
    [else
     ...
     (define flattened
       (for*/vector #:length flattened-size
                    ([node (in-vector slots)]
...
(define (size-subtree node height)
  (cond
    [(fx= height 0) (vector*-length (node-children node))]
    [(node-sizes node)
     =>
     (lambda (sizes) (vector*-ref sizes (fx- (vector*-length sizes) 1)))]
    [else
...
(define (build n height el)
  (cond
    ...
    [else
     ...
     (define child (and (fx> size 0) (build (node-ref n (fx- size 1)) (fx- height 1) el)))
     (cond
       [child
       ...
       [(fx< (node-size n) MAX_WIDTH)
        (Node (vector*-add-right (node-children n) (new-branch el (fx- height 1)))
              (let ([sizes (node-sizes n)])
                (and sizes
                     (vector*-add-right sizes
                                        (fx+ (vector*-ref sizes (fx- (vector*-length sizes) 1))
                                             1)))))]
       [else #false])]))
...
(define (treelist-map tl proc)
  ...
  (for/treelist ([v (in-treelist tl)]) (proc v)))

(define (treelist-for-each tl proc)
  (check-treelist 'treelist-for-each tl)
  (unless (and (procedure? proc) (procedure-arity-includes? proc 1))
    (raise-argument-error* 'treelist-for-each
                           'racket/primitive
...
(define (check-sort-arguments who less-than? get-key)
  ...
  (unless (or (not get-key) (and (procedure? get-key) (procedure-arity-includes? get-key 1)))
    (raise-argument-error* who 'racket/primitive "(procedure-arity-includes/c 1)" get-key)))

(define (treelist-sort tl less-than? #:key [get-key #f] #:cache-keys? [cache-keys? #f])
...
(define-values (prop:treelist-chaperone treelist-chaperone? treelist-chaperone-ref)
  (make-impersonator-property 'treelist))

(struct treelist-wrapper (chaperone? prev props state procs) #:authentic #:sealed)

(struct procs (ref set insert prepend append append2 delete take drop state-key) #:authentic #:sealed)
    ...
    (unless (null? ps)
      ...
      (when (null? (cdr ps))
        (raise-arguments-error* who
                                'racket/primitive
                                "missing value after impersonator-property argument"
...
(define (treelist-chaperone-state tl key [fail-k (make-state-key-fail tl key)])
  ...
  (let loop ([tl tl])
    ...
    (cond
      [(not w) (fail-k)]
      [(eq? key (procs-state-key (treelist-wrapper-procs w))) (treelist-wrapper-state w)]
      [else (loop (treelist-wrapper-prev w))])))
  ...
  (let loop ([tl tl])
    ...
    (cond
      ...
      [else
       ...
       (if new-prev
           (values (re-chaperone new-prev w (treelist-wrapper-state w)) state)
           (values #f #f))])))

(define (make-state-key-fail tl key)
  (lambda ()
    (raise-arguments-error* 'treelist-chaperone-state
                            'racket/primitive
...
(define (re-chaperone new-prev w-in new-state)
  (define w (struct-copy treelist-wrapper w-in [prev new-prev] [state new-state]))
...
(define (treelist-first/slow tl)
  ...
  (check-treelist who tl)
  (cond
    [(treelist-empty? tl) (raise-arguments-error* who 'racket/primitive "treelist is empty")]
    [else (treelist-ref/slow tl 0)]))
...
(define (treelist-last/slow tl)
  ...
  (cond
    [(treelist-empty? tl) (raise-arguments-error* who 'racket/primitive "treelist is empty")]
    [else (treelist-ref/slow tl (fx- (treelist-size tl) 1))]))
...
(define (treelist-rest/slow tl)
  ...
  (cond
    [(treelist-empty? tl) (raise-arguments-error* who 'racket/primitive "treelist is empty")]
    [else (treelist-drop/slow tl 1)]))

(define (treelist-node-for/slow tl index)
  (values (Node (vector (treelist-ref tl index))) 0))
...
(define (call-with-chaperone-values who tl w el producer receiver)
  ...
  (call-with-values
   ...
   (case-lambda
     ...
     [args
      (raise (|#%app| exn:fail:contract:arity
...
(define (treelist-set/slow tl index el)
  ...
  (call-with-chaperone-values
   ...
   w
   el
   (lambda () ((procs-set (treelist-wrapper-procs w)) prev index el (treelist-wrapper-state w)))
...
(define (treelist-insert/slow tl at el)
  ...
  (call-with-chaperone-values
   ...
   w
   el
   (lambda () ((procs-insert (treelist-wrapper-procs w)) prev at el (treelist-wrapper-state w)))
   (lambda (new-el new-state)
...
(define (treelist-do-slow who op procs-op tl at)
  ...
  (define new-state ((procs-op (treelist-wrapper-procs w)) prev at (treelist-wrapper-state w)))
...
(define (treelist-append/slow tl rhs)
  ...
  (cond
    [(impersonator? tl)
     ...
     (define w (treelist-chaperone-ref tl))
     (define prev (treelist-wrapper-prev w))
     (define-values (rhs2 state2)
       (if (procs-append2 (treelist-wrapper-procs w))
           (extract-keyed-treelist rhs (procs-state-key (treelist-wrapper-procs w)))
           (values #f #f)))
     (cond
       [rhs2
        (call-with-chaperone-values
       ...
       [else
        (call-with-chaperone-values
         ...
         w
         rhs
         (lambda () ((procs-append (treelist-wrapper-procs w)) prev rhs (treelist-wrapper-state w)))
         (lambda (new-rhs new-state)
           (define new-tl (treelist-append prev new-rhs))
           (re-chaperone new-tl w new-state)))])]
    [else
     ...
     (call-with-chaperone-values
      ...
      w
      tl
      (lambda () ((procs-prepend (treelist-wrapper-procs w)) tl prev (treelist-wrapper-state w)))
...
(define (treelist-copy-for-mutable tl)
  (cond
    [(eq? tl empty-treelist) tl]
    ;; copy the slow but general way
    [(impersonator? tl) (for/treelist ([i (in-treelist tl)]) i)]
    [else
     (struct-copy treelist
                  tl
                  [root
                   (let copy-node ([n (treelist-root tl)]
                                   [height (treelist-height tl)])
                     (cond
                       [(fx= height 0) (vector*-copy n)]
                       [else
                        (for/vector #:length (vector-length n)
                                    ([e (in-vector n)])
                          (copy-node e (fx- height 1)))]))])]))

(define (treelist-set! tl index el)
  (cond
    [(impersonator? tl) (error "cannot mutate a chaperoned treelist")]
    [else
