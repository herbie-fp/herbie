...
(struct mutable-treelist ([tl #:mutable])
  ...
  (lambda (mtl port mode) (treelist-print "mutable-treelist" (mutable-treelist-tl mtl) port mode))
  #:property prop:equal+hash
  (list (lambda (mtl other-mtl recur)
          (treelist-equal? (mutable-treelist-tl mtl) (mutable-treelist-tl other-mtl) recur))
        (lambda (v recur) (treelist-hash-code (mutable-treelist-tl v) recur))
        (lambda (v recur) (treelist-length (mutable-treelist-tl v))))
  #:property prop:sequence
  (lambda (mtl) (in-mutable-treelist/proc mtl)))
  ...
  (cond
    ...
    [else
     (unless (exact-nonnegative-integer? n)
       (raise-argument-error* 'make-mutable-treelist
                              'racket/primitive
...
(define build-mutable-treelist
  (let ([mutable-treelist (lambda ds (mutable-treelist (list->treelist ds)))]) mutable-treelist))
...
(define (mutable-treelist-snapshot mtl [pos 0] [end-pos #f])
  ...
  (cond
    [(and (eqv? pos 0) (not end-pos)) (treelist-copy-for-mutable tl)]
    [else
...
(define (mutable-treelist-set! mtl index val)
  ...
  (cond
    [(impersonator? mtl) (set-mutable-treelist-tl! mtl (treelist-set tl index val))]
    [else (treelist-set! tl index val)]))
...
(define (mutable-treelist-append! mtl m/tl)
  ...
  (define tl2
    (cond
      ...
      [else
       (raise-argument-error* 'mutable-treelist-append! 'racket/primitive "mutable-treelist?" m/tl)]))
...
(define (mutable-treelist-map! mtl proc)
  ...
  (unless (and (procedure? proc) (procedure-arity-includes? proc 1))
    (raise-argument-error* 'mutable-treelist-map!
                           'racket/primitive
...
(define (mutable-treelist-for-each mtl proc)
  ...
  (unless (and (procedure? proc) (procedure-arity-includes? proc 1))
    (raise-argument-error* 'mutable-treelist-for-each
                           'racket/primitive
                           "(procedure-arity-includes/c 1)"
                           proc))
...
(define (mutable-treelist-member? mtl v [eql? equal?])
  ...
  (unless (and (procedure? eql?) (procedure-arity-includes? eql? 2))
    (raise-argument-error* 'mutable-treelist-member?
                           'racket/primitive
...
(define (mutable-treelist-find mtl match?)
  ...
  (unless (and (procedure? match?) (procedure-arity-includes? match? 1))
    (raise-argument-error* 'mutable-treelist-find
                           'racket/primitive
                           "(procedure-arity-includes/c 1)"
                           match?))
  (treelist-find (mutable-treelist-tl mtl) match?))

(define (mutable-treelist-sort! mtl less-than? #:key [get-key #f] #:cache-keys? [cache-keys? #f])
...
(define-syntax for/mutable-treelist (make-for/treelist #'for/fold/derived #'treelist-copy))
(define-syntax for*/mutable-treelist (make-for/treelist #'for*/fold/derived #'treelist-copy))

(define (check-chaperone-arguments who ref-proc set-proc insert-proc append-proc props)
  (unless (and (procedure? ref-proc) (procedure-arity-includes? ref-proc 3))
    (raise-argument-error* who 'racket/primitive "(procedure-arity-includes/c 3)" ref-proc))
  (unless (and (procedure? set-proc) (procedure-arity-includes? set-proc 3))
    (raise-argument-error* who 'racket/primitive "(procedure-arity-includes/c 3)" set-proc))
  (unless (and (procedure? insert-proc) (procedure-arity-includes? insert-proc 3))
    (raise-argument-error* who 'racket/primitive "(procedure-arity-includes/c 3)" insert-proc))
  (unless (and (procedure? append-proc) (procedure-arity-includes? append-proc 2))
...
(define (chaperone-mutable-treelist mtl
  ...
  (check-chaperone-arguments 'chaperone-mutable-treelist
                             ...
                             append-proc
                             props)
  (chaperone-or-impersonate chaperone-struct
                            chaperone-treelist
                            mtl
                            ref-proc
                            set-proc
...
(define (impersonate-mutable-treelist mtl
  ...
  (check-chaperone-arguments 'impersonate-mutable-treelist
                             ...
                             append-proc
                             props)
  (chaperone-or-impersonate impersonate-struct
...
(define (chaperone-or-impersonate chaperone-struct
                                  chaperone-treelist
                                  mtl
                                  ref-proc
                                  set-proc
                                  ...
                                  append-proc
                                  props)
  (define (tl-ref-proc tl index v state)
    (ref-proc mtl index v))
  (define (tl-set-proc tl index v state)
    (values (set-proc mtl index v) state))
  (define (tl-insert-proc tl index v state)
    (values (insert-proc mtl index v) state))
  (define (tl-prepend-proc lhs tl state)
    (error "prepend should never apply for a mutuable treelist"))
  (define (tl-append-proc tl rhs state)
    (values (append-proc mtl rhs) state))
  (define (tl-delete-proc tl index state)
    state)
  (define (tl-take-proc tl index state)
    state)
  (define (tl-drop-proc tl index state)
    state)
