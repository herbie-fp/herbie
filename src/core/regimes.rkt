    ...
    (for ([point-idx (in-range 0 num-points)])
     ...
     (for ([prev-split-idx (in-range 0 point-idx)])
      ...
      (let ([acost (fl- a-cost min-weight)] 
            ...
            [b-alt-idx 0])
        (define vec-split-costs (make-flvector num-points (fl -1)))
        (flvector-set! vec-split-costs 0 (fl+ (flvector-ref v-alt-cost 0) b-alt-cost))
         ...
         (for ([prev-split-idx (in-range 0 point-idx)])
          (when (vector-ref can-split-vec (+ prev-split-idx 1))
           ...
           (let ([cost (fl- a b)])
            ...
            (set! b-alt-cost cost)
            (set! b-alt-idx cidx)
            (flvector-set! vec-split-costs prev-split-idx (fl+ (flvector-ref v-alt-cost prev-split-idx) b-alt-cost)))))))
      
        (for ([cost (in-flvector vec-split-costs)] [cost-idx (in-naturals)])
         (when (not (= cost -1))
          (when (and (vector-ref can-split-vec (+ cost-idx 1)) (fl< cost acost))
          (set! acost cost)
          (set! a-cost acost)
          (set! a-best b-alt-idx)
          (set! a-prev-idx cost-idx)))))
