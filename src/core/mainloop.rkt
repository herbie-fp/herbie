           ...
           (for ([s (in-list splits)])
             ...
             (when (> pidx prev-pidx)
               (for ([var (in-list vars)]
                     [var-idx (in-naturals)])
                 (define vals
                   (for/list ([i (in-range prev-pidx pidx)])
                     (real->double-flonum (vector-ref (vector-ref pts-vec i) var-idx))))
                 (define finite-vals (filter rational? vals))
                 (when (not (null? finite-vals))
                   (define sorted-vals (sort finite-vals <))
                   (define mid (list-ref sorted-vals (quotient (length sorted-vals) 2)))
                   (when (and (< (abs mid) 1e12) (> (abs mid) 1e-2))
