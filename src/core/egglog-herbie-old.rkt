...
(provide run-egglog-multi-extractor-old)
...
(define (run-egglog-multi-extractor-old runner
                                        output-batch
                                        #:num-variants
                                        [num-variants #t]) ; multi expression extraction
  (printf "egglog herbie\n")
  (define insert-batch
  ...
  ;; 1. Add the Prelude
  (define curr-program (reverse (prelude #:mixed-egraph? #t)))
      ...
      (define tag
        (match rule-type
          ...
          [_
           ...
           (set! curr-program (append curr-program (list `(ruleset ,curr-tag))))
           ...
           (define curr-vals (remove-duplicates (egglog-rewrite-rules rule-type curr-tag)))

           (for ([expr curr-vals])
             (set! curr-program (append curr-program (list expr))))
  ...
  (define-values (new-prog extract-bindings)
    (egglog-add-exprs insert-batch (egglog-runner-ctx runner)))

  (set! curr-program (append curr-program (reverse new-prog)))
  ...
  (set! curr-program (append curr-program (list `(run-schedule ,@run-schedule))))
...
(define (prelude #:mixed-egraph? [mixed-egraph? #t])
  ...
  (set! curr-program (cons spec-egraph curr-program))
  ...
  (set! curr-program (cons typed-graph curr-program))

  ;   (egglog-program-add! `(constructor lower (M String) MTy :unextractable) curr-program)
  (set! curr-program (cons `(constructor lower (M String) MTy :unextractable) curr-program))

  ;   (egglog-program-add! `(constructor lift (MTy) M :unextractable) curr-program)
  (set! curr-program (cons `(constructor lift (MTy) M :unextractable) curr-program))

  ;   (egglog-program-add! `(ruleset const-fold) curr-program)
  (set! curr-program (cons `(ruleset const-fold) curr-program))

  ;   (egglog-program-add! `(ruleset lowering) curr-program)
  (set! curr-program (cons `(ruleset lowering) curr-program))

  ;   (egglog-program-add! `(ruleset lifting) curr-program)
  (set! curr-program (cons `(ruleset lifting) curr-program))

  (for ([curr-expr const-fold])
    ; (egglog-program-add! curr-expr curr-program)
    (set! curr-program (cons curr-expr curr-program)))
  ...
  (set! curr-program (cons (if-lifting-rule) curr-program))

  ;   (egglog-program-add! (approx-lifting-rule) curr-program)
  (set! curr-program (cons (approx-lifting-rule) curr-program))
...
(define (egglog-add-exprs batch ctx)
  ...
  (for ([var (in-list (context-vars ctx))]
    ...
    (set! curr-program (cons curr-var-lowering-rule curr-program)))
  ...
  (for ([var (in-list (context-vars ctx))]
    ...
    (set! curr-program (cons curr-var-lifting-rule curr-program)))
  ...
  (for ([var (in-list (context-vars ctx))])
    ...
    (set! curr-program (cons curr-var-spec-binding curr-program)))
  ...
  (for ([var (in-list (context-vars ctx))]
    ...
    (set! curr-program (cons curr-var-typed-binding curr-program)))
  ...
  (for ([root? (in-vector root-mask)]
    ...
    (set! curr-program (cons curr-binding-exprs curr-program)))
  ...
  (values curr-program extract-bindings))
