...
(module+ test
  ...
  (define test-exprs
    (list (cons '(+.f64 y x) '(+.f64 ($var binary64 $h0) ($var binary64 $h1)))
          (cons '(+.f64 x y) '(+.f64 ($var binary64 $h1) ($var binary64 $h0)))
          (cons '(-.f64 #s(literal 2 binary64) (+.f64 x y))
                '(-.f64 ($literal binary64 2) (+.f64 ($var binary64 $h1) ($var binary64 $h0))))
          (cons '(-.f64 z (+.f64 (+.f64 y #s(literal 2 binary64)) x))
                '(-.f64 ($var binary64 $h2)
                        (+.f64 (+.f64 ($var binary64 $h0) ($literal binary64 2))
                               ($var binary64 $h1))))
          (cons '(*.f64 x y) '(*.f64 ($var binary64 $h1) ($var binary64 $h0)))
          (cons '(+.f64 (*.f64 x y) #s(literal 2 binary64))
                '(+.f64 (*.f64 ($var binary64 $h1) ($var binary64 $h0)) ($literal binary64 2)))
