...
(define-rules arithmetic
  ...
  [div-flip (/ a b) (special-/ 1 (special-/ b a)) #:unsound] ; unsound @ a = 0, b != 0
  [div-flip-rev (/ 1 (/ b a)) (/ a b)])

; Fractions
(define-rules arithmetic
  [sum-to-mult (+ a b) (special-* (+ 1 (special-/ b a)) a) #:unsound] ; unsound @ a = 0, b = 1
...
(define-rules exponents
  ...
  [pow-neg (pow a (neg b)) (special-/ 1 (special-pow a b)) #:unsound]) ; unsound @ a = 0, b = -1
...
(define-rules exponents
  ...
  [log-div (log (/ a b)) (special-- (special-log a) (special-log b)) #:unsound] ; unsound @ a = b = -1
...
(define-rules hyperbolic
  ...
  [cosh-atanh-rev (/ 1 (sqrt (- 1 (* x x)))) (cosh (atanhf x))]
...
(define-rules hyperbolic
  ...
  [sinh-acosh-rev (sqrt (- (* x x) 1)) (sinh (special-acosh x)) #:unsound] ; unsound @ x = -1
