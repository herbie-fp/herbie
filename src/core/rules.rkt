...
;;; Create a module test where given a ruleset, split it into sound/unsound ruels based on lfilter operator set
;;; checker: if you have decalred rule to be sound, then it HAS to be a good operator

;;; So far:
;;;     bad: /, pow, tan
;;;     good: +, *, -, sin, cos, exp, fabs
(define good-operators '(+ * - sin cos exp fabs neg cbrt asin acos PI E < > <= >= not TRUE FALSE))
(define bad-operators '(/ pow tan))

;;; This is a bit of a hack, but this checker goes over all the rulesets and checks that:
;;;   1. The ruleset has the "sound" tag
;;;   2. If the rule is considered (for our case below, uncommented), then every operator in the rule
;;;       is in the set of good operators.

;;; Invariant: (*rules*) == (*included-rules*) + (*excluded-rules*)

(define (*included-rules*)
  (for/list ([r (*rules*)]
             #:when (and (member 'sound (rule-tags r))
                         (in-good-operators? (rule-input r))
                         (in-good-operators? (rule-output r))))
    r))

(define (*excluded-rules*)
  (for/list ([r (*rules*)]
             #:when (or (not (member 'sound (rule-tags r)))
                        (not (in-good-operators? (rule-input r)))
                        (not (in-good-operators? (rule-output r)))))
    r))

(module+ test
  (require rackunit)

  ;; Test 1 : Check that all included rules follow the below conditions
  (for ([r (*included-rules*)])
    (define name (rule-name r))
    (define input (rule-input r))
    (define output (rule-output r))
    (define tags (rule-tags r))

    (with-check-info (['rule-name name] ['input input] ['output output] ['tags tags])
                     (begin
                       ; Check if the rule is declared sound by looking at tags
                       (check-not-false (member 'sound tags))

                       ; Check that if the input has an operator, then it is a good operator
                       (check-not-false (in-good-operators? input))

                       ; Check that if the output has an operator, then it is a good operator
                       (check-not-false (in-good-operators? output)))))

  ;;; Test 2 : Sanity check - union of included and excluded should equal all rules
  (define all-names (map rule-name (*rules*)))
  (define included-names (map rule-name (*included-rules*)))
  (define excluded-names (map rule-name (*excluded-rules*)))

  (define union-names (append included-names excluded-names))

  (check-equal? (list->set all-names) (list->set union-names)))

(define (in-good-operators? expr)
  (let loop ([e expr])
    (match e
      ; Special case for `if` expressions
      [(list 'if _ ift _) (loop ift)]

      ; Check if the operator is a symbol and in the good operators
      ; If it is, then check the arguments recursively
      ; If either is not true, #f is returned
      [(list op args ...) (and (member op good-operators) (andmap loop args))]

      ; A symbol would be a variable so it is just true
      [(? symbol?) #t]

      ; A number would be a constant so it is just true
      [(? number?) #t]

      ; Everything else (so far) is false
      [_ #f])))

(module+ test
  (require rackunit)

  (check-not-false (in-good-operators? '(+ a b)))
  (check-not-false (in-good-operators? '(- (+ a b) c)))

  (check-false (in-good-operators? '(/ a b)))
  (check-false (in-good-operators? '(* (tan a) (pow b))))
  (check-false (in-good-operators? '(- (acosh a b) c)))
  (check-false (in-good-operators? '(- (+ (sin a) (tan b)) c)))

  (check-not-false (in-good-operators? '(if a
                                            x
                                            (if b x y)))))
