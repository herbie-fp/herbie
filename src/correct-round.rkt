...
(require (only-in "syntax/syntax.rkt" operator-info)
         (only-in "common.rkt" *max-mpfr-prec* *sampling-iteration* *max-sampling-iterations* *base-tuning-precision* *ampl-tuning-bits* *sampling-learning-rate* *use-sharing-exponents*)
...
(define (make-progs-interpreter vars ivec rootvec repr)
  ...
  (define iter-count 0)                                ; variable for tracking iteration number from a previous run
  ...
  (define (compiled-spec . args)
    ...
    (match first-iter?
      [#t (when (and (> iter-count 0) (*use-sharing-exponents*))
            ...
            (parameterize ([*sampling-iteration* iter-count])
              (backward-pass ivec varc vregs vprecs vbase-precs rootvec rootlen vrepeats repr))
            (update-vstart-precs vstart-precs vprecs))
          (set! iter-count 0)]
      [#f (backward-pass ivec varc vregs vprecs vstart-precs rootvec rootlen vrepeats repr)
          (set! iter-count (*sampling-iteration*))])
    (timeline-stop!)
...
; Function updates vstart-precs vector by learning from vprecs with *sampling-learning-rate*
; by formula: vstart-precs += ceiling(alpha * (vprecs - vstart-precs))
