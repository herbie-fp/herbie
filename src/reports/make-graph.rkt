...
(define (make-graph-modified result-hash out output? fpcore? profile?)
  (eprintf "make-graph-modified\n")
  (define test (hash-ref result-hash 'test))
  (define backend (hash-ref result-hash 'backend))
  (define job-time (hash-ref result-hash 'time))
   ...
   `(html
     ...
     (body
      ...
      (div ([id "large"])
           ,(render-comparison
             ...
             (format "Minimum Accuracy: ~a → ~a"
                     (format-accuracy (apply max (map ulps->bits start-error)) repr-bits #:unit "%")
                     (format-accuracy (apply max (map ulps->bits end-error)) repr-bits #:unit "%")))
           ,(render-large "Time" (format-time job-time))
           ,(render-large "Alternatives" (~a (length end-alts)))
           ,(if (*pareto-mode*)
                (render-large "Speedup"
                              (if speedup (~r speedup #:precision '(= 1)) "N/A")
      ...
      ,@(for/list ([i (in-naturals 1)]
                   ...
                   [errs end-errors]
                   [cost end-costs])
          (define cur-alt (hash-ref alt-c 'program))
          (define syn (read-syntax 'web (open-input-string cur-alt)))
          (eprintf "~a\n" syn)
          ...
          `(section ([id ,(format "alternative~a" i)] (class "programs"))
                    (h2 "Alternative "
                        ...
                        ,(render-help "report.html#alternatives"))
                    ,body*
                    (details (summary "Derivation") (ol ((class "history")) ,@hist))))
      ,@(for/list ([i (in-naturals 1)]
                   [target (in-list targets)]
                   [target-error (in-list list-target-error)]
                   [target-cost (in-list list-target-cost)])
