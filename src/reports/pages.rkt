...
(require "../syntax/read.rkt" "../syntax/sugar.rkt" "../syntax/syntax.rkt" "../syntax/types.rkt"
         "../utils/alternative.rkt" "../utils/float.rkt" "../core/points.rkt" "../api/sandbox.rkt" "../api/datafile.rkt"
         "common.rkt" "timeline.rkt" "plot.rkt" "make-graph.rkt" "traceback.rkt")
(provide all-pages all-pages-modified make-page
make-page-modified page-error-handler page-error-handler-modified)
...
(define ((page-error-handler-modified result-hash page out) e)
...
|#
(define (make-page-modified page out result-hash output? profile?)
  (define test (hash-ref result-hash 'test))
  ...
  (match page
    ["graph.html"
     (match status
       ['success (make-graph-modified result-hash out output? (get-interactive-js-modified result-hash ctx) profile?)]
       ;; timeout and failure Not fixed yet
    ...
    ["interactive.js"
     (make-interactive-js-modified result-hash out ctx)]
    ["timeline.html"
...
(define (get-interactive-js-modified result-hash ctx)
  ...
  (define start-fpcore (program->fpcore-modified start-expr (hash-ref ctx 'vars) (hash-ref ctx 'repr)))
