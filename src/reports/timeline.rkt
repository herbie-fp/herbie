...
(define/contract (render-phase curr n total-time)
  ...
  `(div ((class ,(format "timeline-block timeline-~a" type)) [id ,(format "timeline~a" n)])
        ...
        (dl ,@(dict-call curr render-phase-memory 'memory 'gc-time)
            ...
            ,@(dict-call curr render-phase-egg-phase 'egg-phase)
            ,@(dict-call curr render-phase-egglog-phase 'egglog-phase)
            ,@(dict-call curr render-phase-egglog-sends 'egglog-send)
...
(define (format-ms ms)
  (string-append (~r (if (real? ms) ms 0.0) #:precision '(= 3)) "ms"))

(define (render-phase-egg-phase rows)
  (define sorted (sort rows > #:key first))
  `((dt "Egg phases")
    (dd (table ((class "times"))
               (tr (th "Phase") (th "Stage") (th "Time (ms)"))
               ,@(for/list ([r (in-list sorted)])
                   (match-define (list time stage phase) r)
                   `(tr (td ,(~a phase)) (td ,(~a stage)) (td ,(format-ms time))))))))

(define (render-phase-egglog-phase rows)
  (define sorted (sort rows > #:key first))
  `((dt "Egglog phases")
    (dd (table ((class "times"))
               (tr (th "Phase") (th "Stage") (th "Wall (ms)"))
               ,@(for/list ([r (in-list sorted)])
                   (match-define (list wall stage phase) r)
                   `(tr (td ,(~a phase)) (td ,(~a stage)) (td ,(format-ms wall))))))))

(define (render-phase-egglog-sends rows)
  (define sorted (sort rows > #:key second))
  `((dt "Egglog sends")
    (dd (details (summary "Per-command histogram (sorted by wall)")
                 (table ((class "times"))
                        (tr (th "Command")
                            (th "Phase")
                            (th "Stage")
                            ...
                            (th "Engine (ms)"))
                        ,@(for/list ([r (in-list sorted)])
                            (match-define (list count wall engine stage phase cmd) r)
                            `(tr (td ,(~a cmd))
                                 (td ,(~a phase))
                                 (td ,(~a stage))
                                 (td ,(~a count))
