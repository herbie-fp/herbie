...
(module+ test
  ...
  (for ([f (in-dict-keys texify-operators)])
    (match-define (list template_ highlight-template_ self-paren-level_ arg-paren-level_) (hash-ref texify-operators f))
    (define-values (self-paren-level arg-paren-level) (precedence-levels f))
    ...
    (define num-args
      (let ([hash (operator-info f 'args)])
        (if (= (length hash) 1)
            (match (car hash) ['* 2] [x x])
            2)))
    (define args (build-list num-args (const "~a")))

    (when (string? template_)
      (check-equal? template_ (apply template args)))
    (check-equal? (apply-converter highlight-template_ args "idx") ((highlight-template f) "idx" args))))
