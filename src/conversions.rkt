  ...
  (let loop ([expr expr] [repr #f])
    (match expr
      ...
      [(list (? rewrite-repr-op? rr) (list (? repr-conv? op) body)) ; repr change on a conversion
       ...
       (if (equal? repr* irepr)
           ...
           (if repr
               ...
               (let* ([conv (get-repr-conv repr* (context-repr ctx))] [body* (loop body repr*)])
      ...
      [(list (? rewrite-repr-op? op) body)
       ...
       (cond
         [(equal? irepr orepr) (loop body irepr)]
         [else
      ...
      [(list 'if con ift iff)
       ...
       (define con*
         (let loop2 ([con con])
           (cond
             [(set-member? '((TRUE) (FALSE)) con) con]
             [else
              (match-define (list op args ...) con)
              (define args*
                (for/list ([arg args] [atype (impl-info op 'itype)])
                  (if (equal? (representation-type atype) 'bool) (loop2 arg) (loop arg atype))))
              (cons op args*)])))
      ...
      [(? literal?) (if repr (literal (literal-value expr) (representation-name repr)) expr)]
      [_ expr])))
