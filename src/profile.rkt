...
(define (profile->json p)
  ...
  (hash 'total-time (exact->inexact (profile-total-time p))
        'cpu-time (exact->inexact (profile-cpu-time p))
        'sample-number (profile-sample-number p)
        'thread-times
        ...
        (for/list ([node nodes])
          (hash 'id (and (node-id node) (~a (node-id node)))
                ...
                'thread-ids (node-thread-ids node)
...
(define (edge->json loc-hash edge)
  (hash 'total (exact->inexact (edge-total edge))
        ...
        'caller-time (exact->inexact (edge-caller-time edge))
        'callee (hash-ref loc-hash (node-loc (edge-callee edge)))
        'callee-time (exact->inexact (edge-callee-time edge))))
    ...
    (set-node-callees! n*
     (for/list ([e (hash-ref n 'callees)])
       (edge (hash-ref e 'total)
             ...
             (hash-ref e 'caller-time)
             (vector-ref nodes (hash-ref e 'callee))
             (hash-ref e 'callee-time))))
    (set-node-callers! n*
     (for/list ([e (hash-ref n 'callers)])
       (edge (hash-ref e 'total)
             ...
             (vector-ref nodes (hash-ref e 'callee))
             (hash-ref e 'callee-time)))))
  (profile (hash-ref j 'total-time)
           (hash-ref j 'cpu-time)
           (hash-ref j 'sample-number)
           (for/list ([t (hash-ref j 'thread-times)])
...
(module+ main
  ...
  (command-line
   #:args (file)
   (render (json->profile (call-with-input-file file read-json)))))
