  ...
  (syntax-case stx (real libm)
    [(_ (operator real ...) real [libm id_d id_f] [key value] ...)
     (let* ([num-args (length (cdr (syntax-e (cadr (syntax-e stx)))))]
            [sym-append (λ args (string->symbol (apply string-append (map symbol->string args))))]
            [op* (syntax-e #'operator)])
       #`(begin
           ...
           (define-operator (#,(sym-append op* '.f64) #,@(build-list num-args (λ (_) #'binary64))) binary64
             [fl (λ args (apply double-proc args))]
             [key value] ...)
           (define-operator (#,(sym-append op* '.f32) #,@(build-list num-args (λ (_) #'binary32))) binary32
             [fl (λ args (apply float-proc args))]
             [key value] ...)
           (declare-parametric-operator/libm (syntax-e #'operator) #,num-args)
       ))]))
