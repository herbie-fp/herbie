...
(define (check-rule-correct test-rule ground-truth)
  ...
  ;; Some rule names are generated by adding a _<num> at the end of the base name. Extract the base name
  ;; so *conditions* doesn't get confused
  (define base-name (string->symbol (first (string-split (symbol->string name) "_"))))

  (define make-point
    (make-sampler
     repr
     `(λ ,fv ,(desugar-program (dict-ref *conditions* base-name 'TRUE) otype var-precs))
     `(λ ,fv ,p1)
     `(λ ,fv ,p2)))
