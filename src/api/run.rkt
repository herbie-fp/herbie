...
(define (run-tests tests #:dir dir #:threads threads)
  ...
  (define long-pole-time (- job-time (/ (+ total-herbie total-profile total-json) (or threads 1))))
  (eprintf "Total time  ~as\n" (format-secs (- (current-inexact-milliseconds) start)))
  (eprintf "  Herbie  ~ax~as\n" (or threads 1) (format-secs (/ total-herbie (or threads 1))))
  (eprintf "  Profile ~ax~as\n" (or threads 1) (format-secs (/ total-profile (or threads 1))))
  (eprintf "  JSONify ~ax~as\n" (or threads 1) (format-secs (/ total-json (or threads 1))))
