...
(define-operations () <binary32>
  ...
  [INFINITY.f32 #:spec (INFINITY) #:fl (const +inf.0) #:fpcore (! :precision binary32 INFINITY) #:cost 32bit-move-cost]
...
(define-operations ([x <binary32>]) <binary32>
  [erfc.f32  #:spec (- 1 (erf x)) #:fl (from-libm 'erfcf)  #:fpcore (! :precision binary32 (erfc x))  #:cost 0.907758]
  [expm1.f32 #:spec (- (exp x) 1) #:fl (from-libm 'expm1f) #:fpcore (! :precision binary32 (expm1 x)) #:cost 0.906484]
  [log1p.f32 #:spec (log (+ 1 x)) #:fl (from-libm 'log1pf) #:fpcore (! :precision binary32 (log1p x)) #:cost 1.302969])

(define-operation (hypot.f32 [x <binary32>] [y <binary32>]) <binary32>
  #:spec (sqrt (+ (* x x) (* y y))) #:fl (from-libm 'hypotf)
...
(define-operations () <binary64>
  [PI.f64   #:spec (PI)       #:fl (const pi)      #:fpcore (! :precision binary64 PI)       #:cost 64bit-move-cost]
  [E.f64    #:spec (E)        #:fl (const (exp 1)) #:fpcore (! :precision binary64 E)        #:cost 64bit-move-cost]
  [INFINITY #:spec (INFINITY) #:fl (const +inf.0)  #:fpcore (! :precision binary64 INFINITY) #:cost 64bit-move-cost]
  [NAN.f64  #:spec (NAN)      #:fl (const +nan.0)  #:fpcore (! :precision binary64 NAN)      #:cost 64bit-move-cost])

;;;;;;;;;;;;;;;;;;;;;;;;;;;;; operators ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define-operation (neg.f64 [x <binary64>]) <binary64>
  #:spec (neg x) #:fl - #:fpcore (! :precision binary64 (- x)) #:cost 0.11567699999999992)

(define-operations ([x <binary64>] [y <binary64>]) <binary64>
  [+.f64 #:spec (+ x y) #:fl + #:fpcore (! :precision binary64 (+ x y)) #:cost 0.200445]
  [-.f64 #:spec (- x y) #:fl - #:fpcore (! :precision binary64 (- x y)) #:cost 0.19106800000000014]
  [*.f64 #:spec (* x y) #:fl * #:fpcore (! :precision binary64 (* x y)) #:cost 0.256602]
  [/.f64 #:spec (/ x y) #:fl / #:fpcore (! :precision binary64 (/ x y)) #:cost 0.3465330000000001])

(define-operations ([x <binary64>] [y <binary64>]) <bool>
  [==.f64 #:spec (== x y) #:fl =          #:fpcore (== x y) #:cost 64bit-move-cost]
  [!=.f64 #:spec (!= x y) #:fl (negate =) #:fpcore (!= x y) #:cost 64bit-move-cost]
  [<.f64  #:spec (< x y)  #:fl <          #:fpcore (< x y)  #:cost 64bit-move-cost]
  [>.f64  #:spec (> x y)  #:fl >          #:fpcore (> x y)  #:cost 64bit-move-cost]
  [<=.f64 #:spec (<= x y) #:fl <=         #:fpcore (<= x y) #:cost 64bit-move-cost]
  [>=.f64 #:spec (>= x y) #:fl >=         #:fpcore (>= x y) #:cost 64bit-move-cost])
...
(define-operations ([x <binary64>]) <binary64>
  [fabs.f64   #:spec (fabs x)   #:fl (from-libm 'fabs)     #:fpcore (! :precision binary64 (fabs x))   #:cost 0.12464599999999992]
  [sin.f64    #:spec (sin x)    #:fl (from-libm 'sin)      #:fpcore (! :precision binary64 (sin x))    #:cost 4.2185290000000003]
  [cos.f64    #:spec (cos x)    #:fl (from-libm 'cos)      #:fpcore (! :precision binary64 (cos x))    #:cost 4.2738829999999994]
  [tan.f64    #:spec (tan x)    #:fl (from-libm 'tan)      #:fpcore (! :precision binary64 (tan x))    #:cost 4.669173000000001]
  [sinh.f64   #:spec (sinh x)   #:fl (from-libm 'sinh)     #:fpcore (! :precision binary64 (sinh x))   #:cost 1.7463029999999996]
  [cosh.f64   #:spec (cosh x)   #:fl (from-libm 'cosh)     #:fpcore (! :precision binary64 (cosh x))   #:cost 1.64949]
  [acos.f64   #:spec (acos x)   #:fl (from-libm 'acos)     #:fpcore (! :precision binary64 (acos x))   #:cost 0.5213330000000001]
  [acosh.f64  #:spec (acosh x)  #:fl (from-libm 'acosh)    #:fpcore (! :precision binary64 (acosh x))  #:cost 0.847705]
  [asin.f64   #:spec (asin x)   #:fl (from-libm 'asin)     #:fpcore (! :precision binary64 (asin x))   #:cost 0.4802670000000002]
  [asinh.f64  #:spec (asinh x)  #:fl (from-libm 'asinh)    #:fpcore (! :precision binary64 (asinh x))  #:cost 1.1207249999999999]
  [atan.f64   #:spec (atan x)   #:fl (from-libm 'atan)     #:fpcore (! :precision binary64 (atan x))   #:cost 1.1015179999999995]
  [atanh.f64  #:spec (atanh x)  #:fl (from-libm 'atanh)    #:fpcore (! :precision binary64 (atanh x))  #:cost 0.4652830000000002]
  [cbrt.f64   #:spec (cbrt x)   #:fl (from-libm 'cbrt)     #:fpcore (! :precision binary64 (cbrt x))   #:cost 1.946652]
  [ceil.f64   #:spec (ceil x)   #:fl (from-libm 'ceil)     #:fpcore (! :precision binary64 (ceil x))   #:cost 0.287118]
  [erf.f64    #:spec (erf x)    #:fl (from-libm 'erf)      #:fpcore (! :precision binary64 (erf x))    #:cost 1.1211180000000001]
  [exp.f64    #:spec (exp x)    #:fl (from-libm 'exp)      #:fpcore (! :precision binary64 (exp x))    #:cost 1.3860560000000002]
  [exp2.f64   #:spec (exp2 x)   #:fl (from-libm 'exp2)     #:fpcore (! :precision binary64 (exp2 x))   #:cost 1.1716009999999999]
  [floor.f64  #:spec (floor x)  #:fl (from-libm 'floor)    #:fpcore (! :precision binary64 (floor x))  #:cost 0.2959660000000004]
  [lgamma.f64 #:spec (lgamma x) #:fl (from-libm 'lgamma)   #:fpcore (! :precision binary64 (lgamma x)) #:cost 2.2206409999999998]
  [log.f64    #:spec (log x)    #:fl (from-libm 'log)      #:fpcore (! :precision binary64 (log x))    #:cost 0.778197]
  [log10.f64  #:spec (log10 x)  #:fl (from-libm 'log10)    #:fpcore (! :precision binary64 (log10 x))  #:cost 1.1793579999999997]
  [log2.f64   #:spec (log2 x)   #:fl (from-libm 'log2)     #:fpcore (! :precision binary64 (log2 x))   #:cost 0.8644160000000004]
  [logb.f64   #:spec (logb x)   #:fl (from-libm 'logb)     #:fpcore (! :precision binary64 (logb x))   #:cost 0.36221]
  [rint.f64   #:spec (rint x)   #:fl (from-libm 'rint)     #:fpcore (! :precision binary64 (rint x))   #:cost 0.29316799999999997]
  [round.f64  #:spec (round x)  #:fl (from-libm 'round)    #:fpcore (! :precision binary64 (round x))  #:cost 0.8677139999999997]
  [sqrt.f64   #:spec (sqrt x)   #:fl (from-libm 'sqrt)     #:fpcore (! :precision binary64 (sqrt x))   #:cost 0.25464700000000007]
  [tanh.f64   #:spec (tanh x)   #:fl (from-libm 'tanh)     #:fpcore (! :precision binary64 (tanh x))   #:cost 1.0567220000000002]
  [tgamma.f64 #:spec (tgamma x) #:fl (from-libm 'tgamma)   #:fpcore (! :precision binary64 (tgamma x)) #:cost 2.6280819999999994]
  [trunc.f64  #:spec (trunc x)  #:fl (from-libm 'trunc)    #:fpcore (! :precision binary64 (trunc x))  #:cost 0.28765399999999997])

(define-operations ([x <binary64>] [y <binary64>]) <binary64>
  [pow.f64       #:spec (pow x y)       #:fl (from-libm 'pow)       #:fpcore (! :precision binary64 (pow x y))       #:cost 2.028296]
  [atan2.f64     #:spec (atan2 x y)     #:fl (from-libm 'atan2)     #:fpcore (! :precision binary64 (atan2 x y))     #:cost 1.9559129999999997]
  [copysign.f64  #:spec (copysign x y)  #:fl (from-libm 'copysign)  #:fpcore (! :precision binary64 (copysign x y))  #:cost 0.2042799999999998]
  [fdim.f64      #:spec (fdim x y)      #:fl (from-libm 'fdim)      #:fpcore (! :precision binary64 (fdim x y))      #:cost 0.7635619999999999]
  [fmax.f64      #:spec (fmax x y)      #:fl (from-libm 'fmax)      #:fpcore (! :precision binary64 (fmax x y))      #:cost 0.23636500000000005]
  [fmin.f64      #:spec (fmin x y)      #:fl (from-libm 'fmin)      #:fpcore (! :precision binary64 (fmin x y))      #:cost 0.24126899999999996]
  [fmod.f64      #:spec (fmod x y)      #:fl (from-libm 'fmod)      #:fpcore (! :precision binary64 (fmod x y))      #:cost 1.7182470000000002]
  [remainder.f64 #:spec (remainder x y) #:fl (from-libm 'remainder) #:fpcore (! :precision binary64 (remainder x y)) #:cost 1.030245])

;;;;;;;;;;;;;;;;;;;;;;;;;;;;; libm accelerators ;;;;;;;;;;;;;;;;;;;;;

(define-operations ([x <binary64>]) <binary64>
  [erfc.f64  #:spec (- 1 (erf x)) #:fl (from-libm 'erfc)  #:fpcore (! :precision binary64 (erfc x))  #:cost 0.907758]
  [expm1.f64 #:spec (- (exp x) 1) #:fl (from-libm 'expm1) #:fpcore (! :precision binary64 (expm1 x)) #:cost 0.906484]
  [log1p.f64 #:spec (log (+ 1 x)) #:fl (from-libm 'log1p) #:fpcore (! :precision binary64 (log1p x)) #:cost 1.302969])

(define-operation (hypot.f64 [x <binary64>] [y <binary64>]) <binary64>
  #:spec (sqrt (+ (* x x) (* y y))) #:fl (from-libm 'hypot)
