...
(define-operations ([x <binary64>]) <binary64> #:fpcore (:precision binary64)
    [degrees.py  #:spec (* x (/ (PI) 180.0)) #:impl (lambda (x)(* x (/ pi 180.0))) #:fpcore (degrees x) #:cost 1]
    [radians.py  #:spec (* x (/ 180.0 (PI))) #:impl (lambda (x)(* x (/ 180.0 pi))) #:fpcore (radians x) #:cost 1]
...
(define-operations ([x <binary64>] [y <binary64>]) <binary64> #:fpcore (:precision binary64)
    ...
    [fsum.2var    #:spec (+ x y) #:impl (lambda (x y) (flsum (list x y)))  #:fpcore (fsum x y) #:cost 1]
...
(define-operations ([x <binary64>] [y <binary64>] [w <binary64>]) <binary64> #:fpcore (:precision binary64)
    [fsum.3var    #:spec (+ (+ x y) w)  #:impl (lambda (x y w) (flsum (list x y w))) #:fpcore (fsum x y w) #:cost 1]
...
(define-operations ([x <binary64>] [y <binary64>] [w <binary64>] [z <binary64>]) <binary64> #:fpcore (:precision binary64)
    [fsum.4var    #:spec (+ (+ (+ x y) z) w)  #:impl (lambda (x y w z) (flsum (list x y w z))) #:fpcore (fsum x y w z) #:cost 1]
...
(define-operations ([x <binary64>] [y <binary64>] [w <binary64>] [z <binary64>] [v <binary64>]) <binary64> #:fpcore (:precision binary64)
    [fsum.5var    #:spec (+ (+ (+ (+ x y) z) w) v)  #:impl (lambda (x y w z v) (flsum (list x y w z v))) #:fpcore (fsum x y w z v) #:cost 1]
