...
(define-operation (-/1 [x <binary64>]) <binary64> #:spec (neg x) #:impl - #:fpcore (- x) #:cost 1)
...
(define-operation (//1 [x <binary64>]) <binary64> #:spec (/ 1 x) #:impl / #:cost 1)

(define-operation (log/2 [x <binary64>] [y <binary64>])
                  <binary64>
                  #:spec (/ (log x) (log y))
                  #:impl log
                  #:fpcore (log x y)
...
(define-operation (flsingle [x <binary64>])
                  <binary64>
                  #:spec x
                  #:impl flsingle
                  #:fpcore (cast x)
