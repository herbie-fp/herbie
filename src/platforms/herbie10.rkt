...
(platform-register-implementation!
 herbie10-platform
 (make-operator-impl (TRUE) bool #:spec (TRUE) #:fl (const true) #:fpcore (! TRUE)))

(platform-register-implementation!
 herbie10-platform
 (make-operator-impl (FALSE) bool #:spec (FALSE) #:fl (const false) #:fpcore (! FALSE)))
...
(platform-register-implementation!
 herbie10-platform
 (make-operator-impl (not [x : bool]) bool #:spec (not x) #:fpcore (not x) #:fl not))
...
(platform-register-implementation!
 herbie10-platform
 (make-operator-impl (or [x : bool] [y : bool]) bool #:spec (or x y) #:fpcore (or x y) #:fl or-fn))
...
; (platform-register-implementation! herbie10-platform fabs.f32)
...
(define libm-impls.f32
  (make-libm-impls/binary32
   ...
   [(binary32 binary32 binary32)
    ([pow 0] [atan2 0] [copysign 0] [fdim 0] [fmax 0] [fmin 0] [fmod 0] [remainder 0])]))
