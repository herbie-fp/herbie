...
(define/contract (render-history altn pcontext pcontext2 ctx)
  ...
  (define err
    (format-bits (errors-score (errors (alt-program altn) pcontext ctx)) repr #:unit "%"))
  (define err2
    (format "Internally ~a" (format-bits (errors-score (errors (alt-program altn) pcontext2 ctx)) repr #:unit "%")))
    ...
    ,@(for/list ([step proof])
        ...
        (define err
          (let ([prog (list 'λ (program-variables prog) expr)])
            (format-bits (errors-score (errors prog pcontext ctx)) repr #:unit "%")))
        `(tr (th ,(if dir
