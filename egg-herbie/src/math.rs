...
const MAX_POLYNOMIAL_TERMS: usize = 64;
const MAX_POLYNOMIAL_DEGREE: u16 = 32;
const MAX_POLYNOMIAL_OPTIONS: usize = 4;
...
impl Polynomial {
    ...
    fn variable(atom: u32) -> Self {
        Self::new(vec![(Monomial(vec![(atom, 1)]), Constant::one())])
    }

    fn new(terms: Vec<(Monomial, Constant)>) -> Self {
    ...
    fn add(&self, other: &Self) -> Option<Self> {
        ...
        for (monomial, coefficient) in self.terms.iter().chain(other.terms.iter()) {
            *terms.entry(monomial.clone()).or_insert_with(Constant::zero) += coefficient;
        }
    ...
    fn mul(&self, other: &Self) -> Option<Self> {
        ...
        for (left_monomial, left_coefficient) in &self.terms {
            for (right_monomial, right_coefficient) in &other.terms {
                ...
                *terms.entry(monomial).or_insert_with(Constant::zero) +=
                    left_coefficient * right_coefficient;
                if terms.len() > MAX_POLYNOMIAL_TERMS {
                    return None;
                }
            }
        }
    ...
    fn from_terms(mut terms: BTreeMap<Monomial, Constant>) -> Option<Self> {
    ...
    fn canonicalize<'a>(&'a self, atom_aliases: &FxHashMap<u32, u32>) -> Cow<'a, Self> {
        ...
        for (monomial, coefficient) in &self.terms {
            ...
            *terms
                ...
                .or_insert_with(Constant::zero) += coefficient;
        }
        terms.retain(|_, coefficient| !coefficient.is_zero());
...
impl Monomial {
    ...
    fn mul(left: &Self, right: &Self) -> Option<Self> {
        ...
        for (atom, exponent) in left.0.iter().chain(right.0.iter()) {
            *variables.entry(atom.clone()).or_insert(0) += exponent;
        }
        let monomial = Self(variables.into_iter().collect());
        (monomial.degree() <= MAX_POLYNOMIAL_DEGREE).then_some(monomial)
    }
}
...
fn polynomials_for(egraph: &mut EGraph, enode: &Math) -> Vec<Polynomial> {
    match enode {
        ...
        Math::Add([a, b]) => combine_polynomial_options(
            &egraph[*a].data.polynomials,
            &egraph[*b].data.polynomials,
            Polynomial::add,
        ),
        Math::Sub([a, b]) => combine_polynomial_options(
            &egraph[*a].data.polynomials,
            &egraph[*b].data.polynomials,
            subtract_polynomials,
        ),
        Math::Mul([a, b]) => combine_polynomial_options(
            &egraph[*a].data.polynomials,
            &egraph[*b].data.polynomials,
            Polynomial::mul,
        ),
        Math::Neg([a]) => normalize_polynomials(
            egraph[*a]
                .data
                .polynomials
                .iter()
...
fn insert_polynomial(options: &mut Vec<Polynomial>, polynomial: Polynomial) {
    if !options.contains(&polynomial) {
        options.push(polynomial);
    }
}

fn normalize_polynomials(mut options: Vec<Polynomial>) -> Vec<Polynomial> {
    options.sort_by_key(Polynomial::cost);
    options.truncate(MAX_POLYNOMIAL_OPTIONS);
    options
}

fn combine_polynomial_options(
    left: &[Polynomial],
    right: &[Polynomial],
    ...
    for left in left {
        for right in right {
            if let Some(polynomial) = operation(left, right) {
                insert_polynomial(&mut options, polynomial);
            }
        }
    }
    normalize_polynomials(options)
}

fn subtract_polynomials(left: &Polynomial, right: &Polynomial) -> Option<Polynomial> {
    left.add(&right.neg())
}
...
impl Analysis<Math> for ConstantFold {
    ...
    fn merge(&mut self, to: &mut Self::Data, from: Self::Data) -> DidMerge {
        ...
        for polynomial in from_polynomials.iter().cloned() {
            insert_polynomial(&mut merged_polynomials, polynomial);
        }
        let merged_polynomials = normalize_polynomials(merged_polynomials);
