use egg::{
    Analysis, BackoffScheduler, CostFunction, EGraph, ENodeOrVar, Extractor, FromOp, Id, Iteration,
    IterationData, Language, PatternAst, RecExpr, Report, Rewrite, RewriteScheduler, Runner,
    SearchMatches, SimpleScheduler, StopReason, Subst,
};
...
#[allow(unused)]
pub fn detour_run<L: Language, N: Analysis<L> + Default, IterData: IterationData<L, N>>(
...
impl<'a, L: Language, N: Analysis<L>, IterData: IterationData<L, N>> Ctxt<'a, L, N, IterData> {
    fn check_limits(&self) -> Result<(), StopReason> {
        ...
        if size > self.limits.node_limit {
...
fn call_hooks<'a, L: Language, N: Analysis<L>, IterData: IterationData<L, N>>(
    ctxt: &mut Ctxt<'a, L, N, IterData>,
) -> Result<(), StopReason> {
    let mut hooks = std::mem::take(&mut ctxt.runner.hooks);
    let res = hooks
        .iter_mut()
        .try_for_each(|hook| hook(&mut ctxt.runner).map_err(StopReason::Other));
    ctxt.runner.hooks = hooks;
    res
}

fn detour_step<'a, L: Language, N: Analysis<L>, IterData: IterationData<L, N>>(
    ctxt: &mut Ctxt<'a, L, N, IterData>,
) -> Result<(), StopReason> {
...
fn pat_detour_eqsat_step<'a, L: Language, N: Analysis<L>, IterData: IterationData<L, N>>(
    ctxt: &mut Ctxt<'a, L, N, IterData>,
) -> Result<(), StopReason> {
    ...
    for (rw_i, rw) in ctxt.rws.iter().enumerate() {
        ...
        for m in ctxt
            .sched
            .search_rewrite(ctxt.runner.iterations.len(), &ctxt.runner.egraph, rw)
        {
            ctxt.check_limits()?;
    ...
    for (full_cost, new_apps) in matches {
        if let Some(found) = found_cost {
            if full_cost > found + ctxt.cfg.offset {
                break;
            }
        }
        for (rw_i, lhs, subst) in &new_apps {
            ...
            rw.applier
                .apply_one(&mut ctxt.runner.egraph, *lhs, subst, pat_ast, rw.name);
            if eg_data(&ctxt.runner.egraph) != og_data && found_cost.is_none() {
...
fn compute_ctxt_costs<'a, L: Language, N: Analysis<L>, IterData: IterationData<L, N>>(
    ex: &Extractor<AdditiveCostFn<L>, L, N>,
    ctxt: &Ctxt<'a, L, N, IterData>,
) -> HashMap<Id, Cost> {
...
fn mk_iteration<'a, L: Language, N: Analysis<L> + Default, IterData: IterationData<L, N>>(
    ctxt: &mut Ctxt<'a, L, N, IterData>,
