...
\begin{abstract}
  ...
  transformations inspired by traditional numerical methods.  \casio's
  search is guided by techniques to estimate rounding error and its
  sources, as well as an approach to detect input regimes with
  distinct error behavior and select the best program variant for each
  regime.  We evaluate \casio on benchmarks drawn from numerical
