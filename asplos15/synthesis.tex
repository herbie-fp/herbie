...
\begin{alltt}
  \textbf{def} local-error (expr : Expr) : \NN \ceq
    match expr with
      c \tto 0
      x \tto 0
      f(\(e_1, \ldots, e_k) \tto
        r_a \ceq f.approx (eval \(e_1\) \textbf{AP}) \ldots (eval \(e_k\) \textbf{AP})
        r_e \ceq f.exact (eval \(e_1\) \textbf{AP}) \ldots (eval \(e_k\) \textbf{AP})
        \(\delta\) r_e r_a
\end{alltt}
