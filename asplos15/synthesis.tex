...
Tools to improve program behavior are typically required to preserve
the semantics of the input program. For \casio this constraint is not
satisfiable: in general, given a program over real numbers, there is
no completely equivalent program over floating numbers due to rounding
error.  Even finding floating point programs with minimal rounding
error is intractable~\cite{fp-np}.  Thus, our first obligation is to
develop a notion of \textit{correctness} for accuracy improving
transformations.

\subsection{Equivalent input and output}

\casio's search is guided by sampling many input points
