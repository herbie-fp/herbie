...
name = "mathematics"
benchmark_dir = "bench/mathematics"
results_dir = f"{name}_results"

subprocess.run(f"mkdir {results_dir}", shell=True, check=True)
subprocess.run(f"Racket -y src/main.rkt report {benchmark_dir} {results_dir}/{name}out/ > {results_dir}/{name}_dump", shell=True, check=True)
